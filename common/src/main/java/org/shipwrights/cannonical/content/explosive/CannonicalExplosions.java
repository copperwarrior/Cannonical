...
public final class CannonicalExplosions {
    ...
    private static final double DEFAULT_BLAST_RADIUS = 4.0D;
    private static final double DEFAULT_IMPACT_POWER = 420.0D;
    private static volatile double impactPower = DEFAULT_IMPACT_POWER;

    private CannonicalExplosions() {
    }
    ...
    public static void detonateGunpowderBarrel(ServerLevel level, double x, double y, double z, Entity source, LivingEntity barrelOwner) {
        ...
        for (Long2DoubleMap.Entry entry : blockImpacts.long2DoubleEntrySet()) {
            ...
            if (blockState.is(ModBlocks.GUNPOWDER_BARREL.get())) {
                ...
                GunpowderBarrelBlock.prime(level, blockPos, barrelOwner, chainFuse);
            ...
            CannonBlockDamageSystem.applyImpact(level, blockPos, blockState, source, resolvedImpactPower, false);
    ...
    private static Long2DoubleOpenHashMap collectRaycastImpacts(ServerLevel level, double centerX, double centerY, double centerZ,
        ...
        int rayCount = Mth.clamp(
                (int) Math.round(DEFAULT_RAY_COUNT * (blastRadius / DEFAULT_BLAST_RADIUS)),
                MIN_RAY_COUNT,
                MAX_RAY_COUNT
        );
    ...
    private static boolean shouldSplitRay(RaycastState ray, double centerX, double centerY, double centerZ, double baseAngularSpacing) {
        ...
        double fromCenterX = ray.x - centerX;
        double fromCenterY = ray.y - centerY;
        double fromCenterZ = ray.z - centerZ;
        double radialDistance = Math.sqrt(
                (fromCenterX * fromCenterX) + (fromCenterY * fromCenterY) + (fromCenterZ * fromCenterZ)
        );
        if (radialDistance <= 1.0E-6D) {
            return false;
        }

        double effectiveAngularSpacing = baseAngularSpacing / Math.sqrt(1 << ray.splitDepth);
        return radialDistance * effectiveAngularSpacing >= RAY_SPLIT_DISTANCE_THRESHOLD;
    }
    ...
    private static Direction perpendicularAxis(double x, double y, double z) {
        ...
        return new Direction(ax / length, ay / length, az / length);
    }
    ...
    private static void applyPostExplosionDamageSmoothing(ServerLevel level, Long2ByteOpenHashMap survivingDamage) {
        ...
        for (int pass = 0; pass < POST_SMOOTH_PASSES; pass++) {
            ...
            for (Long2ByteMap.Entry entry : smoothedTargets.long2ByteEntrySet()) {
                ...
                CannonBlockDamageSystem.ImpactResult result = CannonBlockDamageSystem.accumulateTransferredDamageState(
                        level,
                        blockPos,
                        blockState,
                        entry.getByteValue(),
    ...
    public static void setImpactPower(double newImpactPower) {
        impactPower = Math.max(1.0D, newImpactPower);
    }
    ...
    public static double getDefaultImpactPower() {
        return DEFAULT_IMPACT_POWER;
    }

    public static double getBlastRadius() {
        double normalized = Math.max(0.01D, impactPower / DEFAULT_IMPACT_POWER);
